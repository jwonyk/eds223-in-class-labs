...
filelist <- list.files(here::here("data", "landsat-data", "landsat-data"), full.names = TRUE)

# read in and store as a raster stack
landsat <- rast(filelist)

# Update layer names to match bonds
...
SB_county_south <- st_read(here::here("data", 
                                      "landsat-data", 
                                      "SB_county_south.shp")) %>%
...
# Mask the raster to the southern portion of SB county
...
landsat <- (landsat * .0000275 - 0.2) * 100
...
training_data <- st_read(here::here( "data", "landsat-data","trainingdata.shp")) %>%
...
```{r}
# Extract the pixel values from Landsat raster at the locations in the training data
...
# Join training data attribute and extracted reflectance values
...
spectral_data <- landsat_df[, c("blue", "green", "red", 
                                "NIR", "SWIR1", "SWIR2")]

# Scale the data
spectral_data_scaled <- scale(spectral_data)

view(spectral_data_scaled)
```
...
values(SB_classification_kmeans) <- NA # Assign all values in that raster as NA

# Take the (x, y) coordinate from `landsat_df`
# Convert them to cell numbers for the raster
...
# Cluster 3 = Green Vegetation
# Cluster 4 = Water
# Cluster 1 = Soil / Dead Grass
# Cluster 2 = Urban
