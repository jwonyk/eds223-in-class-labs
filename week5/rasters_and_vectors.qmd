...
```{r}
tm_shape(elevation) +
  tm_raster(col.legend = tm_legend(title = "Elvation (meters)")) +
  tm_shape(boundary) +
  tm_borders(lwd = 2)
```
...
# (Cell outside of vector are converted to NA)
...
# Convert high elevation pixels into polygon shapes and the transform into a sf
...
```{r}
# Define bounding box coordinates for our area of interest
...
base_map = bryce_zoom_mat %>% 
  height_shade() %>%
  add_overlay(sphere_shade(bryce_zoom_mat, 
                           texture = "desert", 
                           colorintensity = 5), alphalayer = 0.5) %>%
  add_shadow(lamb_shade(bryce_zoom_mat), 0) %>%
  add_shadow(ambient_shade(bryce_zoom_mat), 0) %>% 
  add_shadow(texture_shade(bryce_zoom_mat,
                           detail = 8/10,
                           contrast = 9,
                           brightness = 11), 0.1)
...
# Query OSM for all "highway" features (roads, paths, trails)
...
```{r}
# Create a combine trails layer with different tyles for each type
