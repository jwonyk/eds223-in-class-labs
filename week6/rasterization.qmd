...
if(crs(elevation) == crs(boundary)) {
  ...
  warning("Updating coordinating reference to match")
...
```{r}
# Update park boundary object name to clarify its a polygon
park_polygon <- boundary

# Create line version of park boundary
park_border <- boundary %>% 
  st_cast(. , "MULTILINESTRING") # Dot is syntactical sugar ;D
```
...
template_raster_low <- rast(ext(park_polygon), resolution = 0.05,
...
tmap_arrange(map1, map2, ndvi_map, nrow = 2)
```
...
tmap_arrange(map1, map3, map_ndwi, nrow = 2)
```

Normalize Difference Moisture Index: Vegetation index used in RS to measure and monitor the water content in vegetation

NDMI = (NIR - SWIR) / (NIR + SWIR)

- Measuring moisture within plant levels:
...
```{r}
#| message: false
# Define NDMI as the normalized difference between NIR and SWIR bands
